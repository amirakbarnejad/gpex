...
class AkResNet18(AkResNet):
    def __init__(self, num_classes=9, dim_wideoutput=10):
        super(AkResNet18, self).__init__(BasicBlock, [2, 2, 2, 2], num_classes, dim_wideoutput)
...
def AkResNet34():
    return AkResNet(BasicBlock, [3, 4, 6, 3])


def AkResNet50():
    return AkResNet(Bottleneck, [3, 4, 6, 3])


def AkResNet101():
    return AkResNet(Bottleneck, [3, 4, 23, 3])


def AkResNet152():
    return AkResNet(Bottleneck, [3, 8, 36, 3])
...
class AkResNetBackbone(nn.Module):
    def __init__(self, block, num_blocks, dim_before_wideoutput=None, input_size=32, dim_wideoutput=None, flag_setdimoutput_to_one = False):
        super(AkResNetBackbone, self).__init__()
        self.in_planes = 64
        self.block = block
...
class ResnetClassifierWithAttention(nn.Module):
    def __init__(
        ...
        self.backbone_classification = AkResNetBackbone(
        ...
        self.module_attention = AkResNetBackbone(
